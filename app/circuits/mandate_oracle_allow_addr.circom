...
// WARRANT mandate predicate with recipient identity bound as
// (type, 32-byte key split into two big-endian 128-bit limbs).
template MandateOracleAllowAddr(depth) {
    // ---- public inputs ----
    ...
    recipientType * (recipientType - 1) === 0;
    ...
    leaf.inputs[0] <== recipientType;
    leaf.inputs[1] <== recipientHi;
    leaf.inputs[2] <== recipientLo;
...
component main {public [policyCommitment, prevStateRoot, nextStateRoot, amount, recipientType, recipientHi, recipientLo, price]} = MandateOracleAllowAddr(3);
